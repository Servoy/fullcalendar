...
function ResourceEventRenderer() {
    ...
    function renderEvents(events, modifiedEventId) {
        ...
        if (opt('allDaySlot')) {
            ...
            renderDayEvents(cloneCompiledDayEvents(compileDaySegs(dayEvents)), modifiedEventId, resources);
    ...
    /* PA fix
     * render does expect to receive events
     * to correctly calculate the allDay segments width in the resource view
     * all events are being cloned and start/end day are modified to correctly match the resource view column
     * issue: if allDay is assigned to true, the width is not correctly calculated
     ---------------------------------------------*/
    function cloneCompiledDayEvents(segments) {
    	var events = [];
    	for (var j=0; j < segments.length; j++) {
    		var seg = segments[j];
    		var event = seg.event;
			var cloneEvent = $.extend( false, {}, event );
			cloneEvent._isClone = true;
			cloneEvent.start = seg.start;
			cloneEvent.end = seg.end;
			cloneEvent._start = seg.start;
			cloneEvent._end = seg.end;
			cloneEvent.allDay = undefined;
    		events.push(cloneEvent)
    	}
    	return events
    }
	
    // The same function exists in AgendaEventRender
    function segCmp(a, b) {
